...
└── init.ts              # 四种入口共用的初始化（被 main.tsx 的 preAction 调用）
```
...
如果你不是在终端里用 `claude`，而是在自己的 Node / Bun 程序里 `import { query, createSdkMcpServer, unstable_v2_createSession } from '@anthropic-ai/claude-agent-sdk'`，那么真正映射到的就是 `entrypoints/agentSdkTypes.ts`（443 行）。这个文件本身**不实现**任何函数 —— 它的所有具名 export 都 `throw new Error('not implemented')`（见 agentSdkTypes.ts:87 / 106 / 121 / 132 / 144 / 164 等），真正的实现在打包时由 SDK 的 bundle 注入。

它的角色是 **公共 API 表面**：

- 从 `sdk/coreTypes.js` 重新导出可序列化的消息 / 配置类型
- 从 `sdk/runtimeTypes.js` 重新导出 callbacks 与带方法的接口
- 从 `sdk/controlTypes.js` 暴露 `SDKControlRequest / SDKControlResponse`（带 `@alpha` 标注，给 bridge 子路径的高级用户）
- 重新导出 `Settings`（由 settings JSON schema 自动生成）和所有 `toolTypes`
...
第四种入口看起来最不像入口 —— `sandboxTypes.ts` 只导出 Zod schema：`SandboxNetworkConfigSchema`、`SandboxFilesystemConfigSchema`、`SandboxSettingsSchema`，以及它们 infer 出来的 TS 类型。但它确实是一种入口形态：当 `claude` 以 sandbox 模式启动子命令时，使用的就是这一份 schema 校验出来的配置（`enabled` / `failIfUnavailable` / `autoAllowBashIfSandboxed` / `allowUnsandboxedCommands` / `network` / `filesystem` / `enableWeakerNetworkIsolation` 等），并以这份配置为锚去约束子进程的网络 / 文件系统访问。
