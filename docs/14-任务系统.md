...
| `tools/TaskOutputTool/` | TaskState（运行期） | `tools/TaskOutputTool/TaskOutputTool.tsx:144-308` | 默认启用（别名 `AgentOutputTool`/`BashOutputTool`），prompt 标记 `[Deprecated]`，建议改用 `Read` 读 outputFile |
...
`TaskOutputTool` 则是另一种姿态。它的 prompt 第一行就明写着 `[Deprecated] — prefer Read on the task output file path`：模型应该优先用通用的 `Read` 工具读 `TaskState.outputFile` 路径，而不是再调用一个专门的工具来拉日志。`TaskOutputTool` 仍然保留是因为别名 `AgentOutputTool`/`BashOutputTool` 还活在历史 transcripts 里——一旦移除，老 session resume 时会报"找不到工具"。它的 `call()` 提供了 `block=true` 阻塞模式（等任务进入终态后再返回输出）和 `block=false` 轮询模式（立即返回当前 status）两种调用方式，对应 `tools/TaskOutputTool/TaskOutputTool.tsx:208-282`。
