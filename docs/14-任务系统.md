...
新增章节: no
风格双亲: docs/03-状态管理.md + docs/13-内置Agent设计模式.md
---
...
读到这里你可能已经发现一件事：源码里有两层都叫 "task"。一层是前文反复讲的 **TaskState**（`Task.ts` 里那 7 个 wire 字面量），它管的是"后台跑了哪些进程/Agent/会话"；另一层是 `utils/tasks.ts` 里的 **TodoV2 任务列表**，它管的是"这一轮对话里我打算分几步做"——是给模型自己整理思路用的 todo 看板。这两套体系互不相关，但都对模型暴露了名字以 `Task` 开头的工具，刚开始读源码很容易混淆。

`tools/` 下与任务相关的目录正好 6 个，按归属切两半：

| 工具目录 | 目标 | 入口源码 | 启用门 |
|---|---|---|---|
| `tools/TaskStopTool/` | TaskState（运行期） | `tools/TaskStopTool/TaskStopTool.ts:39-131` | 默认启用（别名 `KillShell`，向后兼容） |
| `tools/TaskOutputTool/` | TaskState（运行期） | `tools/TaskOutputTool/TaskOutputTool.tsx:144-308` | 默认启用（别名 `AgentOutputTool`/`BashOutputTool`），`description()` 与 `prompt()` 均标记 deprecated，建议改用 `Read` 读 outputFile |
| `tools/TaskCreateTool/` | TodoV2 看板 | `tools/TaskCreateTool/TaskCreateTool.ts:48-138` | `isTodoV2Enabled()` |
| `tools/TaskGetTool/` | TodoV2 看板 | `tools/TaskGetTool/TaskGetTool.ts:38-128` | `isTodoV2Enabled()` |
| `tools/TaskListTool/` | TodoV2 看板 | `tools/TaskListTool/TaskListTool.ts:33-116` | `isTodoV2Enabled()` |
| `tools/TaskUpdateTool/` | TodoV2 看板 | `tools/TaskUpdateTool/TaskUpdateTool.ts:88-406` | `isTodoV2Enabled()` |

### 4.1 TaskStopTool 与 TaskOutputTool：对接前面的 TaskState

`TaskStopTool` 是模型主动 kill 后台任务的入口。它的 `call()` 本质是一个薄壳，把工作整个委派给 `tasks/stopTask.ts:38-100` 中的 `stopTask()`：
...
`stopTask()` 内部做三件事：(1) 按 `task.type` 通过 `getTaskByType()` 找到对应的 `Task` 实现；(2) 调 `taskImpl.kill()`；(3) 对 `local_bash` 走一条额外的"通知抑制"分支——`bash` 被 kill 时正常会走通知队列，但 exit code 137 本身没有信息量，所以源码在这里直接把 `notified` 翻为 true，跳过 XML 通知，改为通过 `emitTaskTerminatedSdk()` 给 SDK 消费者发一条精简事件。注释解释了原因：
...
`TaskOutputTool` 则是另一种姿态。它的 `description()` 与 `prompt()` 都被显式标 deprecated：`tools/TaskOutputTool/TaskOutputTool.tsx:157-159` 的 `description()` 返回 `'[Deprecated] — prefer Read on the task output file path'`，`tools/TaskOutputTool/TaskOutputTool.tsx:172-181` 的 `prompt()` 首行写的是 `DEPRECATED: Prefer using the Read tool on the task's output file path instead.`。两处共同表达同一个倾向：模型应该优先用通用的 `Read` 工具读 `TaskState.outputFile` 路径，而不是再调用一个专门的工具来拉日志。`TaskOutputTool` 仍然保留是因为别名 `AgentOutputTool`/`BashOutputTool` 还活在历史 transcripts 里——一旦移除，老 session resume 时会报"找不到工具"。它的 `call()` 提供了 `block=true` 阻塞模式（等任务进入终态后再返回输出）和 `block=false` 轮询模式（立即返回当前 status）两种调用方式，对应 `tools/TaskOutputTool/TaskOutputTool.tsx:208-282`。

这两个工具共同对应的，正是第二节讲的 `framework.ts` 与第三节列举的具体 TaskState 实现：它们不创建任务（任务由 BashTool 后台化、AgentTool spawn 等业务工具创建），只负责"看"和"杀"。

### 4.2 TaskCreate/Get/List/Update：另一套并行的 TodoV2 体系

剩下 4 个工具——TaskCreate、TaskGet、TaskList、TaskUpdate——名字里都有 task，但**完全不碰** `AppState.tasks`。它们走的是 `utils/tasks.ts` 提供的 TodoV2 接口（`createTask` / `getTask` / `listTasks` / `updateTask` / `blockTask` / `deleteTask`），背后是按 `taskListId` 持久化的 JSON 文件，每条 task 的字段是 `subject` / `description` / `status` / `owner` / `blocks` / `blockedBy` / `metadata` 等。

这套体系是给模型自己整理思路用的 todo 看板，**status 用的是另一套字面量**：`pending` / `in_progress` / `completed` / 以及更新时可设为 `deleted`，与 TaskState 的 `pending|running|completed|failed|killed` 是两套独立的状态机。

四个工具的职责很对称：

- **TaskCreateTool**（`tools/TaskCreateTool/TaskCreateTool.ts:80-129`）创建一条 todo，同时跑 `executeTaskCreatedHooks()`——如果 hook 抛出 blocking error，刚创建的 task 会被 `deleteTask()` 回滚。创建后会把 UI 的 `expandedView` 切到 `tasks`，强制把面板展开。
- **TaskListTool**（`tools/TaskListTool/TaskListTool.ts:65-90`）只读拉清单，过滤掉 `metadata._internal` 的内部 task，并把每条 task 的 `blockedBy` 里已 completed 的 ID 剔掉——避免"被一个已完成的任务阻塞"这种噪音。
- **TaskGetTool**（`tools/TaskGetTool/TaskGetTool.ts:73-97`）按 id 取单条，包含完整的 `description` / `blocks` / `blockedBy`。
- **TaskUpdateTool**（`tools/TaskUpdateTool/TaskUpdateTool.ts:123-363`）做更新，是这套体系里最复杂的一个：除了改字段，还要在 `status = completed` 时跑 `executeTaskCompletedHooks()`；在 swarm 模式下，把 `in_progress` 的 task 自动归属当前 agent；如果 owner 发生变化，向新 owner 的 mailbox（`utils/teammateMailbox.ts`）写一条 `task_assignment` 消息，让被指派的 teammate 在自己的轮次里看到。
...
当主线程 agent 一次性关掉一个 3+ 条的 todo list、且其中没有任何一条是"verification"步时，工具结果里会被追加一段提示，要求 agent 在收尾前显式 spawn `subagent_type = VERIFICATION_AGENT_TYPE` 的子 Agent 来做验证。这个反 "summary 自评" 的结构性约束，写在工具层而不是 prompt 里，逃不掉。

### 4.3 为什么放在同一章

把这两套体系放在同一章讲，不是因为它们共享代码——它们几乎不共享任何代码——而是因为它们共享同一个**词**："task"。读源码时，区分 `AppState.tasks` 与 `utils/tasks.ts` 是绕不开的第一道坎。TaskState 是运行时的 process-supervisor 模型，TodoV2 是模型自我组织的看板模型；前者由业务工具（BashTool、AgentTool 等）隐式创建、由 TaskStop/TaskOutput 显式观察，后者由模型自己拿着 4 个 Task*Tool 主动写入。两者的字面量、状态机、持久化层、UI 渲染路径都不一样。
