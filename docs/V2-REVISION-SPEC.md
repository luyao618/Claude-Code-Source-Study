...
| 写作公约 | 无 | **§0 五公约**（强制） |
| CI 校验 | 无 | **7 项脚本**（source_commit 一致性 / 程度副词禁词 / manifest diff & 孤儿目录 / diff 体积闸 / 标题保留闸 / 代码块占比闸 / 工程化小标题禁词） |
...
3. **字数与代码占比对齐 v1**：v1 已发布章节的散文：代码块字符比大致在 4:1 到 6:1 之间。新章必须落在这个区间。代码占比超 25% 触发 CI fail（C-3）。**为使 CI 能稳定识别"新章"，这 8 篇文件 frontmatter 必须显式声明 `新增章节: yes`**（实际文件采用 `NN-标题.md` 命名，文件名中不再嵌入书脊编号 `C04 / C13`，CI 仅以 frontmatter 为唯一判据）。
...
2. **开篇被换皮**：v1 的"当你面对一个约 1900 个源码文件……最大的挑战不是读懂某个函数，而是不知道从哪里开始读"这种叙事破题，被替换为 `## 源码锚点` 列表（主入口、关键类型、关键函数、数字来源）。违反 §0.5.2 第 1 条与 §0.5.3 第 2.4 条。**修正**：保留 v1 开篇 3 段，源码锚点改为正文末尾的"附：本章源码引用清单"或脚注，**不**置顶。
...
- **R-1 风格双亲声明**：PR 描述含"风格双亲：v1-XX + v1-YY"两章名。
...
> **新章 frontmatter 强制**：8 篇新增章节（C04 / C13 / C17 / C24 / C25 / C28 / C29 / C30）的 markdown 文件 frontmatter 必须包含 `新增章节: yes`，否则 CI C-3 闸（代码块占比）不会对其生效——v2 实际文件采用 `NN-标题.md` 命名，文件名内不嵌入书脊编号，frontmatter 是 CI 识别"新章"的唯一判据。
...
body:
  spec:
    ...
    is_new_chapter: <true|false>    # 8 篇新章=true；写入文件 frontmatter 为 `新增章节: yes`
