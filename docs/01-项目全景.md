...
> **v2 补记：12 个值得入图的"新成员"。** 自 v1 写作以来对源码的再一次深扫发现，上面这张图并未把所有一级目录都纳入，至少还有 12 个目录值得提一下，它们对应 v2 新增的 8 个章节。具体来说：`bridge/` 与 `remote/` 撑起了"手机/Web/Desktop 远程控制本地 CLI session"这条完整链路；`coordinator/` 则把多 Agent 编排从 `tasks/` 里独立了出来；`server/` 与 `upstreamproxy/` 分管 DirectConnect 与上游代理，对应企业内网拓扑；`migrations/` 把"Settings schema 演化"沉淀为可执行的迁移脚本；`buddy/` 引入了人格化伙伴模块；`screens/` 装着 Doctor、REPL、ResumeConversation 这些屏级组件；`outputStyles/` 让"输出风格"成为与 Skill / Plugin 并列的第三条扩展路径；`keybindings/`、`vim/`、`voice/` 三者一起，在终端里复刻出一台带模态编辑与语音输入的编辑器；`native-ts/` 用原生 TS 重写了 yoga-layout 等性能热点；`schemas/` 集中存放 Hooks 与配置的 Zod schema。这 12 个目录在 v1 时要么尚未浮现，要么未被提到一级模块的高度，v2 会在 C04（migrations）、C13（通信调度工具）、C17（coordinator）、C24（bridge/remote）、C25（DirectConnect/upstream proxy）、C28（keybindings/vim/voice）、C29（buddy）、C30（Doctor + Output Style 体验）这 8 章里逐一展开。
